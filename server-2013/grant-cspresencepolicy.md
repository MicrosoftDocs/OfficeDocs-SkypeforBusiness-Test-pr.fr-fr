...
ms:mtpsurl: https://technet.microsoft.com/fr-fr/library/Gg398571(v=OCS.15)
ms:contentKeyID: 49297746
ms.date: 05/20/2016
...
Grants a per-user presence policy to a user or group of users. Cette applet de commande est une nouveauté de Lync Server 2010.
...
In Example 2, the presence policy RedmondPresencePolicy is assigned to all the users who have accounts in the Redmond OU in services de domaine Active Directory. To do this, the command first uses the **Get-CsUser** cmdlet and the OU parameter to return a collection of all the user accounts found in the Redmond OU (OU=Redmond,dc=litwareinc,dc=com). This collection is then piped to the **Grant-CsPresencePolicy** cmdlet, which assigns the policy RedmondPresencePolicy to each user in the collection.
