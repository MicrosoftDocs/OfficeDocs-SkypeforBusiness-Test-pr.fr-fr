...
ms:mtpsurl: https://technet.microsoft.com/fr-fr/library/Gg398786(v=OCS.15)
ms:contentKeyID: 49298245
ms.date: 05/20/2016
...
Enables you to remove an existing collection of configuration settings applied to computers running the service Edge d’accès (these computers are also known as A/V Edge servers). An A/V Edge server enables internal users to share audio and video data with external users (that is, users who are not logged on to your internal network). Cette applet de commande est une nouveauté de Lync Server 2010.
