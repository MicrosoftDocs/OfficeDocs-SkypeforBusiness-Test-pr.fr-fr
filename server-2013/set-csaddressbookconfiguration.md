...
ms:mtpsurl: https://technet.microsoft.com/fr-fr/library/Gg412784(v=OCS.15)
ms:contentKeyID: 49298433
ms.date: 05/20/2016
...
Modifies an existing collection of Address Book configuration settings. Cette applet de commande est une nouveauté de Lync Server 2010.
...
Address Book servers are intermediaries between AD DS and Lync Server. The Address Book server ensures that the user information stored in Lync Server is in synch with the user information stored in AD DS. This is done by periodically synching Address Book files with information stored in the base de données utilisateur.
...
<td><p>When set to True (the default value) the Address Book server generates Address Book index files that can be downloaded by clients. When set to False, these index files are not generated. That means that client applications will have to use the service de requête sur le web du carnet d’adresses when searching for contacts.</p></td>
</tr>
<tr class="odd">
...
<td><p>Indicates how often Address Book servers synchronize their information with the information stored in the base de données utilisateur. The SynchronizePollingInterval can be set to any value between 5 seconds (00:00:05) and 3 hours (03:00:00). The default value is 5 minutes (00:05:00).</p></td>
</tr>
<tr class="odd">
