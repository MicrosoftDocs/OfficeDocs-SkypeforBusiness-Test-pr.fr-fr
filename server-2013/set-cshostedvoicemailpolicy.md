...
ms:mtpsurl: https://technet.microsoft.com/fr-fr/library/Gg412722(v=OCS.15)
ms:contentKeyID: 49298313
ms.date: 05/20/2016
...
Modifies a hosted voice mail policy. Cette applet de commande est une nouveauté de Lync Server 2010.
...
This command modifies the Destination property of a hosted voice mail policy named ExRedmond. It sets the messagerie unifiée Exchange destination for this policy to be at FQDN ExUM.contoso.com.
...
This cmdlet modifies a policy that configures a user account enabled for Lync Server or Microsoft Office Communications Server to use an messagerie unifiée Exchange hosted voice mail service. The policy determines how to route unanswered calls to the user to a hosted messagerie unifiée Exchange service.
...
<td><p>Permet de transmettre une référence à un objet à la cmdlet plutôt que de définir des valeurs de paramètre individuelles. The object must be of type HostedVoicemailPolicy and can be retrieved by calling the <strong>Get-CsHostedVoicemailPolicy</strong> cmdlet.</p></td>
</tr>
<tr class="odd">
...
<td><p>Globally unique identifier (GUID) of the Skype Entreprise Online tenant account for which hosted voicemail policy being modified. For example:</p>
