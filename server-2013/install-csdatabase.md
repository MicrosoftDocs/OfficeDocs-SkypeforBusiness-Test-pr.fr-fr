...
ms:mtpsurl: https://technet.microsoft.com/fr-fr/library/Gg399044(v=OCS.15)
ms:contentKeyID: 49299196
ms.date: 05/20/2016
...
Installs one or more Lync Server databases. Cette applet de commande est une nouveauté de Lync Server 2010.
...
The command shown in Example 2 installs the magasin central de gestion on the pool atl-sql-001.litwareinc.com. The database will be installed in the rtc instance, and make use of the folder G:\\CSDB.
...
Lync Server makes extensive use of SQL Server databases, ranging from the magasin central de gestion to the base de données d’archivage. As a general rule, these databases are set up at the same time you install Lync Server or at the same time you install a Lync Server role (such as serveur de surveillance) that requires a database back end. After installation has taken place these databases typically will not need to be reinstalled or moved to new locations.
...
<td><p>If this parameter is included, the <strong>Install-CsDatabase</strong> cmdlet will use the SqlServerFqdn parameter to install the magasin central de gestion on the specified computer. This parameter is typically used only by Générateur de topologie, and is generally called just once, during initial setup.</p></td>
</tr>
<tr class="even">
...
<td><p>Optional</p></td>
<td><p>System.Management.Automation.SwitchParameter</p></td>
<td><p>If present, additional database roles will be collocated with the magasin central de gestion.</p></td>
</tr>
<tr class="odd">
