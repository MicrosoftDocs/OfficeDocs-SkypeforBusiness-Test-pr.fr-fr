...
ms:mtpsurl: https://technet.microsoft.com/fr-fr/library/Gg425949(v=OCS.15)
ms:contentKeyID: 49297084
ms.date: 07/07/2016
...
_**Dernière rubrique modifiée :** 2016-07-07_

Creates a new client policy. Among other things, client policies help determine the features of Lync Server that are made available to users; for example, you might give some users the right to transfer files while denying this right to other users. Cette applet de commande est une nouveauté de Lync Server 2010.
...
<td><p>Indicates how users are allowed to access information by using the service de requête sur le web du carnet d’adresses and/or by downloading a copy of the Address Book to their local computer). AddressBookAvailability must be set to one of the following values:</p>
...
<tbody>
<tr class="odd">
<td>When using a Skype Entreprise client, with either Office 2013 or Office 2016, this policy won't work in the same way.<br />
In that combination, the Skype Entreprise client uses the search capabilities of Office (mso.dll), which finds contacts from Exchange mailbox contact folders. There isn't an option to suppress the search of those contact folders for the Office search component.</td>
</tr>
</tbody>
</table>
