...
ms:mtpsurl: https://technet.microsoft.com/fr-fr/library/JJ205238(v=OCS.15)
ms:contentKeyID: 49298741
ms.date: 05/20/2016
...
Returns information about the Open Authorization (OAuth) servers configured for use by the organization. OAuth servers, also known as security token servers, issue security tokens used in server-to-server authentication and authorization. Cette applet de commande est une nouveauté de Lync Server 2013.
...
**Panneau de configuration Lync Server:** The functions carried out by the **Get-CsOAuthServer** cmdlet are not available in the Panneau de configuration Lync Server.
...
<td><p>Globally unique identifier (GUID) of the Skype Entreprise Online tenant account whose OAuth server settings are to be retrieved.</p>
<p>For example:</p>
