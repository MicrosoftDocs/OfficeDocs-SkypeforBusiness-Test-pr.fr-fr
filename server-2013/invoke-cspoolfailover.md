...
ms:mtpsurl: https://technet.microsoft.com/fr-fr/library/JJ205189(v=OCS.15)
ms:contentKeyID: 49298595
ms.date: 05/20/2016
...
Invokes the failover process for a Lync Server 2013 pool. Failover refers to the process that occurs when a pool fails and the current users of that pool are then signed on to a backup pool. Cette applet de commande est une nouveauté de Lync Server 2013.
...
**Panneau de configuration Lync Server:** The functions carried out by the **Invoke-CsPoolFailOver** cmdlet are not available in the Panneau de configuration Lync Server.
